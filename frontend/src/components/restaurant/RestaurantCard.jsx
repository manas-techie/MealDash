...
function RestaurantCard({ restaurant }) {
  ...
  return (
    <article className="group overflow-hidden rounded-[28px] border border-white/10 bg-white/5 shadow-2xl shadow-black/15 transition duration-200 hover:-translate-y-1 hover:border-orange-300/30 hover:bg-white/[0.07]">
        ...
        {imageUrl ? (
          ...
          <div
            className={`absolute inset-0 bg-linear-to-br ${
              restaurant.gradient || "from-orange-500 via-amber-400 to-rose-400"
            }`}
          />
        )}
      ...
      <div className="space-y-5 p-6">
        ...
        <div className="flex items-center justify-between gap-4 border-t border-white/10 pt-5">
          ...
          <Link
            to={restaurant.href}
            className="inline-flex items-center justify-center rounded-full bg-white px-4 py-2.5 text-sm font-bold text-slate-950 transition hover:scale-[1.02]"
          >
            View details
          </Link>
        </div>
